...
where t.branch_id = d.house_id
 ...
 {and d.house_id = ?} order by t.application_date desc'
...
UPDATE TP_C_SQLINFO s
 ...
 where {t.branch_id in (select h.house_id from TB_P_HOUSE h, TB_C_HOUSEACCOUNT t  where h.staff_id = t.houseaccount_name and  t.staff_id like ''%'' || {?} ||''%'' {and h.housename like ''%'' || ? || ''%''}) {and t.mobile like ''%'' || ? || ''%'' } {and
      to_char(t.application_date, ''yyyy/MM/dd'') >= ? } {and
      ? >= to_char(t.application_date, ''yyyy/MM/dd'') } {and
      t.equipment = ? } {and t.status = ? } order by t.application_date desc'
